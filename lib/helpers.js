...
// department
const retForDepart = function (array){
    let newArray = [];
    for (let i = 0; i < array.length; i++) {
...
const inquirerFormatEmpOpt = function (array){
    let newArray = [];
    for (let i = 0; i < array.length; i++) {
        const {id,first_name, last_name, manager_id} = array[i];
        if (first_name != null) {
            newArray.push({name:`${first_name} ${last_name}`,value:`${id === undefined ? manager_id : id}` });
        }
...
module.exports = {returnTitleOnly,getIdOfValue,returnIdwithNameDisplayed,inquirerFormatEmpOpt,inquirerFormatDepartments,retForDepart}